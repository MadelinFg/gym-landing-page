    ...
    let productFeatures = [
        ...
        },
        {
            featureList: [
                ...
                "Guarantees progress on all core excercises",
            ],
...
<SectionWrapper id="product">
    ...
    >
        <div class="flex flex-col gap-2">
            <p class="opacity-600 text-base sm:text-lg md:text-xl text-center">
        ...
        <ProductCard {productFeature} {index}>
            {#if index === 0}
            Expience a beginner excecise <span class="text-indigo-400 font-medium">master</span> class
            {:else if index === 1}
            Lay the foundations <span class="text-indigo-400 font-medium">routine</span> and consistency
            {:else}
